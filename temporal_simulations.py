...
#
lam0, lam = 400.0, 800.0 # wavelength range in nm
...
#sky, wls = SIM.sim_sky_squid(lam0,lam,nm_sample) # Generates the sky scene
#obj = 'squid'
...
#sky,wls = SIM.sim_sky_old_gals(lam0,lam,nm_sample)
#obj = 'old_gals'
#flux_factor = 0.05

#sky,wls = SIM.sim_sky_gal_cluster(lam0,lam,nm_sample)
#obj = 'gal_cluster'
#flux_factor = 0.25
#sky, wls = SIM.sim_sky_cygnus(lam0,lam,nm_sample) # Generates the sky scene
#obj = 'cygnus'
#
#sky, wls = SIM.sim_sky_globular(lam0,lam,nm_sample) # Generates the sky scene
#obj = 'globular'
# flux_factor = 0.07

#sky,wls = SIM.sim_sky_agn2(lam0,lam,nm_sample)
#obj='AGN2'
#flux_factor = 0.055
...
##sky,wls = SIM.sim_sky_spiral(lam0,lam,nm_sample)
##obj='spiral'
##flux_factor = 0.00000000000003

sky, wls = SIM.sim_sky_galaxy(lam0,lam,nm_sample) # Generates the sky scene
obj = 'galaxy'
flux_factor = 0.015 #0.055
#%% Finding magnitude
...
#%% Creating input sky scene data cube
n = np.shape(sky[0])
...
#%%Untouched HTSI (No temporal variations)
if mat_type == 'H':
    htsi_data2, sky_ADU2, htsi_mse2,ccdN, phoN = HTSI.htsi(sky1, order, slit_width, DMD_contrast, CCD)
if mat_type == 'S':
    htsi_data2, sky_ADU2, htsi_mse2, ccdN, phoN = HTSI.S_htsi(sky1, order, slit_width, DMD_contrast, CCD)
mos_data2, sky_ADU_mos2, mos_mse2,ccdN, phoN = HTSI.MOS(sky1, order, slit_width, DMD_contrast, CCD)
...
#logfile.write('SSIM sky: '+str(np.mean(h_mse))+'\n')  
#logfile.write('SSIM htsi: '+str(np.mean(h_mse))+'\n')  
#logfile.write('SSIM htsi temporal: '+str(np.mean(h_mse))+'\n')  
#logfile.write('SSIM mos: '+str(np.mean(h_mse))+'\n')  
#logfile.write('SSIM mos temporal: '+str(np.mean(h_mse))+'\n')  
...
wl_slice = 257
b = 130 # slice in spatial axis
xs, ys = 125,128# spectrum points

plt.subplot(3,5,1)
plt.imshow(sky_ADU2[:,:,wl_slice],clim = (0,110),cmap = 'inferno')
plt.title('Original Sky')
plt.axis('off')
plt.subplot(3,5,2)
plt.imshow(htsi_data[:,:,wl_slice],clim = (0,110),cmap = 'inferno')
plt.title('HTSI')
plt.axis('off')
plt.subplot(3,5,3)
plt.imshow(htsi_data2[:,:,wl_slice],clim = (0,110),cmap = 'inferno')
plt.title('HTSI without temporal variations')
plt.axis('off')
plt.subplot(3,5,4)
plt.imshow(mos_data[:,:,wl_slice],clim = (0,110),cmap = 'inferno')
plt.title('Long Slit Scan')
plt.axis('off')
plt.subplot(3,5,5)
plt.imshow(mos_data2[:,:,wl_slice],clim = (0,110),cmap = 'inferno')
plt.title('Scan without temporal variations')
plt.axis('off')

plt.subplot(3,5,6)
plt.imshow(sky_ADU[b,:,:],clim = (0,110),cmap = 'inferno')
plt.axis('off')
plt.subplot(3,5,7)
plt.imshow(htsi_data[b,:,:],clim = (0,110),cmap = 'inferno')
plt.axis('off')
plt.subplot(3,5,8)
plt.imshow(htsi_data2[b,:,:],clim = (0,110),cmap = 'inferno')
plt.axis('off')
plt.subplot(3,5,9)
plt.imshow(mos_data[b,:,:],clim = (0,110),cmap = 'inferno')
plt.axis('off')
plt.subplot(3,5,10)
plt.imshow(mos_data2[b,:,:],clim = (0,110),cmap = 'inferno')
plt.axis('off')
#
plt.subplot(3,5,11)
...
plt.subplot(1,5,1)
plt.imshow(sky_ADU2[:,:,wl_slice],clim = (0,100),cmap = 'inferno')
plt.title('Original Sky')
plt.axis('off')
plt.subplot(1,5,2)
plt.imshow(htsi_data[:,:,wl_slice],clim = (0,100),cmap = 'inferno')
plt.title('HTSI')
plt.axis('off')
plt.subplot(1,5,3)
plt.imshow(htsi_data2[:,:,wl_slice],clim = (0,100),cmap = 'inferno')
plt.title('HTSI without temporal variations')
plt.axis('off')
plt.subplot(1,5,4)
plt.imshow(mos_data[:,:,wl_slice],clim = (0,100),cmap = 'inferno')
plt.title('Long Slit Scan')
plt.axis('off')
plt.subplot(1,5,5)
plt.imshow(mos_data2[:,:,wl_slice],clim = (0,100),cmap = 'inferno')
plt.title('Scan without temporal variations')
plt.axis('off')
#